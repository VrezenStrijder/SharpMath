...
using System.Text;
using SharpMath.Core.Parsers.IndependentParser;
...
namespace SharpMath.Test
{
    public class ExpressionCase
    {
        public static void Case1()
        {
            ...
            // 示例5：注册自定义数学运算符
            // 阶乘运算符
            parser.RegisterUnaryOperator("!",4, (arg) =>{
                var factorialMethod = typeof(CustomMath).GetMethod("Factorial");
                return Expression.Call(factorialMethod, arg);
            });
            var expr5 = parser.ParseWithVariables("x!", "x");
            var func5 = (Func<double, double>)expr5.Compile();
            Console.WriteLine("Sample5:");
            Console.WriteLine($"6! = {func5(6)}");
